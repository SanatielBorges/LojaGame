...
const FpsGamesGrid = ({
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://api.rawg.io/api/games?key=${apiKey}&genres=shooter`)
      .then((response) => {
        const gamesWithPrices = response.data.results.map((game) => ({
          ...game,
          img: game.background_image, // Certifica-se de que img está presente
          title: game.name, // Certifica-se de que title está presente
          price: `R$ ${(Math.random() * (200 - 100) + 100)
            .toFixed(2)
            .replace(".", ",")}`, // Preço aleatório entre R$100 e R$200
        }));
